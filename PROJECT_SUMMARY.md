...
### AI / ML
- **LLM**: LGAI-EXAONE/EXAONE-3.5-2.4B-Instruct (HuggingFace Transformers, 로컬 GPU)
- **임베딩**: BAAI/bge-m3 (SentenceTransformer)
- **라우터 분류 모델**: klue/roberta-base 파인튜닝 (4-class 텍스트 분류)
- **LLM 프레임워크**: LangChain (RunnableWithMessageHistory — 멀티턴 대화)
...
추천 요청 시 유사 상품 top15 검색 → source 기준 중복 제거 → 고유 상품 3개 전체 chunk 수집 → EXAONE 3.5 프롬프트 → 구조화된 추천 결과 파싱.

### 3. 특약 멀티턴 상담
LangChain `RunnableWithMessageHistory`로 대화 이력을 유지하며 특약 추가/제거를 누적 반영.
세션별 현재 특약 목록 상태를 서버에서 관리.
...
[Agent 분기]
    ├─ RecommendationAgent
    │      ↓ RAG 검색 (ChromaDB / BAAI/bge-m3)
    │      ↓ 전체 chunk 수집
    │      ↓ EXAONE 3.5 추론
    │      ↓ 구조화 파싱
    │
    ├─ ProductInfoAgent
    │      ↓ source 특정 → 전체 chunk → EXAONE 정리
    │
    ├─ SpecialTermsAgent
    │      ↓ LangChain 멀티턴 (RunnableWithMessageHistory)
    │      ↓ EXAONE 추론 → 특약 추가/제거 파싱
    │
    └─ GeneralQAAgent
           ↓ RAG top3 → EXAONE 답변
    ↓
...
- pdfplumber 기반 보험 약관 PDF 파싱 및 의미단위 청킹 → 벡터 DB 구축
- LangChain RunnableWithMessageHistory 적용으로 특약 멀티턴 상담 기능 구현
